...
# Connect to Facebook Oauth
@app.route('/fbconnect', methods=['POST'])
def fbconnect():
    if request.args.get('state') != login_session['state']:
        response = make_response(json.dumps('''Invali d state parameter.''',
                                            401))
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    # Save Login details to Login Session
...
# -------------------------------------------------
# Disconnect from login session on Google or Facebook
# -------------------------------------------------
...
# -------------------------------------------------
# JSON end points for Restaurant Catalog
# -------------------------------------------------
    ...
    if request.method == 'POST':
        if (
            request.form['name'] != '' and
                request.form['name'] != editedRestaurant.name):
            editedRestaurant.name = request.form['name']
            flash('Restaurant Successfully Edited %s' % editedRestaurant.name)
            return redirect(url_for('showRestaurants'))
        else:
            return redirect(
                   url_for('editRestaurant', restaurant_id=restaurant_id))
    else:
        return render_template(
...
def deleteRestaurant(restaurant_id):
    ...
    if request.method == 'POST':
        menuItems = session.query(
                    MenuItem).filter_by(
                    restaurant_id=restaurantToDelete.id).all()
...
def newMenuItem(restaurant_id):
    ...
    if request.method == 'POST':
        if (
            ...
            return redirect(
                   url_for('newMenuItem', restaurant_id=restaurant_id))
    else:
        return render_template(
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if request.method == 'POST':
        ...
        if (
            request.form['description'] and
                request.form['description'] != editedItem.description):
            checkChange = True
            editedItem.description = request.form['description']
        if request.form['price'] and request.form['price'] != editedItem.price:
            checkChange = True
            editedItem.price = request.form['price']
        if (
            request.form['course'] and
                request.form['course'] != editedItem.course):
            checkChange = True
